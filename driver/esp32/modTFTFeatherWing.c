...
STATIC bool ts_read(lv_indev_drv_t * drv, lv_indev_data_t * data) {
   ...
   if (is_touched(self)) {
	 ...
	 read_xyz(self, &y, &x, &z);
      ...
      data->point.x = TFT_WIDTH - (((x - TS_MINX) * TFT_WIDTH) / (TS_MAXX - TS_MINX)) ;
      data->point.y = (((y - TS_MINY) * TFT_HEIGHT) / (TS_MAXY - TS_MINY));
      data->state = LV_INDEV_STATE_PR;

      self->last_x = data->point.x;
      self->last_y = data->point.y;
   } else {
